...
var (
		commanderInstanceLock sync.Once
		commander             *CommandLineArgsProviderImpl
)

func CommandLineArgsProviderOf() CommandLineArgsProvider {
		if commander == nil {
				commanderInstanceLock.Do(commandLineProviderNew)
		}
		return commander
}

func commandLineProviderNew() {
		commander = new(CommandLineArgsProviderImpl)
}
